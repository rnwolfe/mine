...
description: JSON communication contract for mine plugins
---
...
Plugins declare their capabilities in a `mine-plugin.toml` manifest:
...
[[commands]]
name = "sync"
description = "Sync vault to remote"
```
...
sequenceDiagram
    ...
    User->>mine: mine todo add "buy milk" -p high
    mine->>Plugin: JSON context via stdin
    Note over Plugin: Transform context<br/>(add tags, modify flags)
    Plugin->>mine: Modified context via stdout
    mine->>mine: Execute command with modified context
    mine->>User: Display result
```
...
See the [mine-plugin-template](https://github.com/rnwolfe/mine-plugin-template) repository for a starter template in Go.

Plugins can be written in any language that can:
1. Read JSON from stdin
2. Write JSON to stdout
3. Exit with appropriate status codes

Popular choices: Go, Python, Node.js, Rust, Bash (with `jq`)
